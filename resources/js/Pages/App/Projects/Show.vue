        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-4">
                    <Link :href="route('dashboard')" class="text-gray-400 hover:text-gray-600 transition" :title="$t('project.back_dashboard')">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm text-red-700">
                        {{ $t('project.crawling_failed') }}. <span v-if="tarefa.message">{{ tarefa.message }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white border rounded-lg shadow-sm overflow-hidden">
                <div class="bg-[#5bc0de] text-white text-center py-2 font-bold uppercase tracking-wider">{{ $t('project.pages_discovered') }}</div>
                <div class="p-6 flex justify-between items-center">
                    <div>
                        <span class="text-4xl font-bold text-[#31708f]">{{ tarefa.pages_count || 0 }}</span>
                        <div class="text-xs font-bold text-gray-400 uppercase mt-1">{{ $t('project.total_indexed') }}</div>
                    </div>
                     <div class="text-right text-xs text-gray-600 space-y-1">
                        <div><span class="font-bold">{{ (tarefa.pages_count || 0) + 12 }}</span> {{ $t('project.stat_discovered') }}</div>
                        <div class="text-green-600"><span class="font-bold">{{ tarefa.pages_count || 0 }}</span> {{ $t('project.new_added') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border rounded-lg shadow-sm overflow-hidden opacity-75">
                <div class="bg-[#d2d6de] text-gray-600 text-center py-2 font-bold uppercase tracking-wider">{{ $t('project.images') }}</div>
                <div class="p-6 text-center">
                     ...
                     <span class="mt-2 inline-block bg-green-500 text-white text-[10px] px-2 py-0.5 rounded uppercase font-bold">{{ $t('project.pro_feature') }}</span>
                </div>
            </div>

            <!-- Videos Card -->
            <div class="bg-white border rounded-lg shadow-sm overflow-hidden opacity-75">
                <div class="bg-[#d2d6de] text-gray-600 text-center py-2 font-bold uppercase tracking-wider">{{ $t('project.videos') }}</div>
                <div class="p-6 text-center">
                     <span class="text-4xl font-bold text-gray-400">0</span>
                     <div class="text-xs font-bold text-gray-400 uppercase mt-1">{{ $t('project.stat_indexed') }}</div>
                      <span class="mt-2 inline-block bg-green-500 text-white text-[10px] px-2 py-0.5 rounded uppercase font-bold">{{ $t('project.pro_feature') }}</span>
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden min-h-[500px]">
            ...
            <div class="p-8">
                ...
                <div v-if="abaAtiva === 'details'">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="text-sm text-gray-500">
                            {{ $t('project.showing_recent') }} <a href="#" class="text-blue-500 hover:underline">{{ $t('project.download_full_list') }}</a>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto border rounded-lg">
                        <table class="w-full text-left text-sm text-gray-500">
                             <thead class="bg-gray-50 border-b border-gray-200 text-xs uppercase font-bold text-gray-600">
                                <tr>
                                    <th class="px-6 py-3">{{ $t('project.location') }}</th>
                                    <th class="px-6 py-3 text-right">{{ $t('project.last_mod') }}</th>
                                    <th class="px-6 py-3 text-right">{{ $t('project.priority') }}</th>
                                    <th class="px-6 py-3 text-right">{{ $t('project.change_freq') }}</th>
                                </tr>
                            </thead>
                ...
                <div v-else-if="abaAtiva === 'files'">
                    ...
                    <h3 class="text-lg font-bold text-gray-700 mb-4">{{ $t('project.available_downloads') }}</h3>
                         ...
                         <div v-for="arq in arquivosMapeados" :key="arq.name" class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-md transition bg-white group">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ arq.count }} {{ $t('project.urls_inside') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <a :href="arq.url" target="_blank" class="text-gray-400 hover:text-gray-600 p-2" :title="$t('project.view_action')">

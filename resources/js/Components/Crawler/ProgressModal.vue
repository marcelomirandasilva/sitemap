            ...
            <div class="bg-white p-8 border-t border-gray-200 relative min-h-[400px]">
                ...
                <div v-else-if="tarefa?.status === 'completed' && abaAtiva === 'details'">
                    ...
                    <div class="flex gap-4 mb-4">
                        ...
                        <button @click="$emit('close')" class="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold text-xs uppercase py-2 px-4 rounded shadow-sm flex items-center gap-2">
                            {{ $t('modal.close') }}
                        </button>
                    </div>
                ...
                <div v-else-if="tarefa?.status === 'completed' && abaAtiva === 'files'">
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded p-4 mb-6 text-sm text-gray-600">
                        <span class="font-bold">{{ $t('crawler.note_label') }}</span> {{ $t('crawler.note_text') }} <br>
                        {{ $t('crawler.upgrade_text') }}
                    </div>

                    <table class="w-full text-left border-t border-b border-gray-200">
                        <thead>
                            <tr class="border-b border-gray-200">
                                <th class="py-3 font-bold text-gray-700 text-sm text-center">{{ $t('crawler.th_view') }}</th>
                                <th class="py-3 font-bold text-gray-700 text-sm text-center">{{ $t('crawler.th_download') }}</th>
                                <th class="py-3 font-bold text-gray-700 text-sm text-center">{{ $t('crawler.th_convert') }}</th>
                            </tr>
                        </thead>
